#!/bin/bash
# Driver script for autonomous MySQL recursive descent parser implementation.
...
echo "=== MySQL Recursive Descent Parser Builder ==="
...
" 2>&1) || EXIT_CODE=$?
    ...
    if [ "$EXIT_CODE" -eq 42 ]; then
        ...
        cd "$OMNI_DIR" && go test ./mysql/... 2>&1 | tee "$LOG_DIR/final_verification.log"
        exit 0
    fi

    echo "Starting iteration $ITERATION..."

    # Random sleep to avoid git lock contention with other engine pipelines
    sleep $(( (RANDOM % 3) + 1 ))
