...
- `oracle/parser/eval_error_adversarial_test.go` — adversarial tests go in this **separate** file (never modify `eval_error_test.go`)

## Rules

1. **Analysis ONLY** — do NOT modify any non-test `.go` implementation file.
2. Do NOT modify existing test functions. Write adversarial tests in `oracle/parser/eval_error_adversarial_test.go` (separate file).
...
```json
{
  "version": 1,
  "patterns": [
    {"name": "pattern-name", "file": "stage4-pattern-name.md", "severity": "high|medium|low", "stage": 4}
  ]
}
```
...
If the analysis reveals edge cases not covered by eval tests, write new test functions in `oracle/parser/eval_error_adversarial_test.go` (never modify `eval_error_test.go`):
...
cd /Users/rebeliceyang/Github/omni && go test -v -count=1 ./oracle/parser/ -run "TestEvalStage4" -skip "Adversarial" -timeout 300s
...
cd /Users/rebeliceyang/Github/omni && go test -v -count=1 ./oracle/parser/ -run "TestEvalStage4_Adversarial" -timeout 300s || true
```
